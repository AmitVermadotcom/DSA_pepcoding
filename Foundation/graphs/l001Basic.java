...
public class l001Basic {
    ...
    public static boolean isGraphconnected(ArrayList<Edge>[] graph ,boolean[] vis) {
        int components = 0;
   ...
   public static void hamintonian_dfs(ArrayList<Edge>[] graph,int src, int osrc, boolean[] vis, int NoOfEdges, String psf) {
        if (NoOfEdges == vis.length - 1) {
        ...
        for (Edge e : graph[src]) {
            if (!vis[e.nbr])
                hamintonian_dfs(graph,e.nbr, osrc, vis, NoOfEdges + 1, psf + src);
        }
    ...
    public static int journeyToMoon(int n, int[][] astronaut) {
        ...
        for (int i = 0; i < n; i++) {
            if (!vis[i])
                sizeArr.add(moon_dfs(graph, i, vis));
        }
