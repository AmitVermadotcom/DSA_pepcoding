...
//   863. All Nodes Distance K in Binary Tree
    ...
    public static ArrayList<Integer> VerticalOrderSum(TreeNode root){
        ...
        while(que.size() != 0){
            ...
            while(size-->0){
                ...
                int lv = rn.vl;
                ans.set(lv,ans.get(lv) + node.val);
    ...
    // vertical order sum using recursion and dubly linkedlist
    ...
    public static ArrayList<Integer> VerticalOrderSum_01(TreeNode root){

        ListNode node = new ListNode(0);
        VerticalOrderSum_01Helper(root,node);
        while(node.prev != null) node=node.prev;
        ArrayList<Integer> ans=new ArrayList<>();
        while(node.next != null) ans.add(node.val);
        return ans;
    }

    public static void VerticalOrderSum_01Helper(TreeNode root,ListNode node){
        if(root == null) return;
        node.val += root.val;
        if(root.left != null){
            if(node.prev == null){
            ...
            VerticalOrderSum_01Helper(root.left,node.prev);
        }
        if(root.right != null){
            if(node.next == null){
                ListNode curr = new ListNode(0);
            ...
            VerticalOrderSum_01Helper(root.right,node.next);
        } 
    }

    public static ArrayList<Integer> diagonalOrderSum_01(TreeNode root){
        ListNode node=new ListNode(0);
        diagonalOrderSum_01Helper(root,node);
        while(node.prev != null) node=node.prev;
        ArrayList<Integer> ans=new ArrayList<>();
        while(node.next != null) ans.add(node.val);
        return ans;
    ...
    public static void diagonalOrderSum_01Helper(TreeNode root,ListNode node){
        if(root == null) return;
        node.val += root.val;
        if(root.left != null){
            if(node.prev == null) {
            ...
            diagonalOrderSum_01Helper(root.left,node.prev);
        }
        diagonalOrderSum_01Helper(root.right,node);
    }
    ...
    public static void allSOlutionBT(TreeNode root,TreeNode data,allSolPair pair){
        if(root == null) return;
        allSOlutionBT(root.left,data);
        if(root.val == data.val){
            pair.pred = prev;
        }
        if(pair.prev != null && pair.prev.val == data.val){
            pair.succ=root;
            return;
        }
        pair.prev=root;
        allSOlutionBT(root.right,data,pair);
    }
}